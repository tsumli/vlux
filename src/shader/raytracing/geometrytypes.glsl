...
// coordinates
Triangle UnpackTriangle(const uint index) {
    Triangle tri;
    const uint tri_index = index * 3;

    Indices indices = Indices(buffer_references.indices);
    Vertices vertices = Vertices(buffer_references.vertices);

    // Unpack vertices
    // Data is packed as vec4 so we can map to the glTF vertex structure from the host side
    for (uint i = 0; i < 3; i++) {
        const uint vertex_offset = uint(indices.i[tri_index + i]) * 3;
